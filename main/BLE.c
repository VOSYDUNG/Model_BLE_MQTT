...
#define SAMPLE_DEVICE_NAME          "LOP_1_NHOM_4"
...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0E, 0x09, 'L', 'O', 'P', '_', '1', '_','N', 'H', 'O', 'M','_', '4'
};
            ...
            if (!param->write.is_prep){
                ...
                if (gatt_db_handle_table[IDX_CHAR_CFG_C_2] == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                        ...
                        ESP_LOGI(EXAMPLE_TAG, "(6) ***** Read data from user %s ***** \n",(char*)mqtt_data);
                    }
                }
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle && param->write.len == 4) {
                        
                        memset(receive, 0, sizeof(receive));
                        memcpy(receive, param->write.value, param->write.len);
                        ESP_LOGI(EXAMPLE_TAG, "(3)***** short write success ***** \n");
                        mqtt_app_start();
                        // Ngắt kết nối Bluetooth (nếu cần thiết)
                }
